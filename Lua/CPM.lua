...
function FireworkExplosion(Position: UDim2)
	...
	local MiddlePoints = MoveAndConvertUniform2DPoints(getPointsFromUniform2D({Resolution = 250, Points = VectorPoints}, 50), Position) -- Get points in a circle with a radius of 50 from the uniform 2d
	...
	for i,v in pairs(AllPoints) do -- Loop through all points
		...
		local diff = Udim2ToVector2(v) - frame.AbsolutePosition
		...
		goal.Position = UDim2.fromOffset(frame.AbsolutePosition.X + direction.X * speed, frame.AbsolutePosition.Y + direction.Y * speed)
...
function LaunchParticle(Type: string)
	...
	local GoalPosX = math.random(LaunchPosX-(.1 * ResolutionX),LaunchPosX+(.1 * ResolutionX)) -- get another random position on x with a max of .1 distance from the original position
	...
	local randomSize = --[[random:NextNumber(0.0047, 0.0055)]] 0.0055 -- used to be a random size (You can see the old code for it) removed because funky on mobile
	templateFrame.Size = UDim2.new(0,randomSize * ResolutionY,0,randomSize * ResolutionY)
	templateFrame.BorderSizePixel = 0
	...
	local function fire() -- Fire function (cause of delta time)
		...
		local Pos = UDim2.new(0,math.random(lerp(LaunchPosX, GoalPosX, percent)-7, lerp(LaunchPosX, GoalPosX, percent)+7),0, currentheight) -- get the position where the frame is supposed to be (lerps between the x launch pos and the goal launch pos so it rotates a little)
		...
		local Time = random:NextNumber(.3, .65) -- random time between .3 and .65 (lifetime)
		...
		if side == 1 then -- if the side is 1 then move it to the left
			Goal.Position = UDim2.new(0,math.random(Pos.X.Offset - 25, Pos.X.Offset - 9), 0, stateHeight+55)
		else -- if the side is 2 then move it to the right
			Goal.Position = UDim2.new(0,math.random(Pos.X.Offset + 9, Pos.X.Offset + 25), 0, stateHeight+55)
		end

		local Info = TweenInfo.new(Time, Enum.EasingStyle.Linear, Enum.EasingDirection.In) -- tweeninfo, linear and in were not needed
	...
	repeat
		...
		for i=1, iterations do -- create a frame for each iteration
			if currentheight < height then break end
			currentheight -= ResolutionY*.0125
			fire()
		end
...
function cpm.Fireworks(Amount: number, Interval: number) -- function used to launch the fireworks
	...
	for i = 1, Amount do -- loop until the amount of fireworks specified have all been created
